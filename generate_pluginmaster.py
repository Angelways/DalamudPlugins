"""Generate the custom Dalamud plugin repository manifest from packaged plugins."""
...
def load_manifest(zip_path: Path, plugin_name: str) -> dict:
    ...
    if manifest.get("InternalName") != plugin_name:
        raise RuntimeError(
            f"{zip_path}: InternalName={manifest.get('InternalName')!r}，"
            f"应为 {plugin_name!r}"
        )
...
def trim_manifest(manifest: dict) -> dict:
    return {key: manifest[key] for key in MANIFEST_KEYS if key in manifest}
...
def last_update(zip_path: Path) -> str:
    try:
        dirty = subprocess.run(
            ["git", "status", "--porcelain", "--", zip_path.as_posix()],
            check=True,
            capture_output=True,
            text=True,
        ).stdout.strip()
        if not dirty:
            committed = subprocess.run(
                ["git", "log", "-1", "--format=%ct", "--", zip_path.as_posix()],
                check=True,
                capture_output=True,
                text=True,
            ).stdout.strip()
...
def build_entry(zip_path: Path, plugin_name: str) -> dict:
    manifest = trim_manifest(load_manifest(zip_path, plugin_name))
    url = download_url(plugin_name)
...
def extract_manifests() -> list[dict]:
    ...
    for plugin_dir in sorted(path for path in PLUGINS_ROOT.iterdir() if path.is_dir()):
        ...
        for variant_dir in sorted(path for path in plugin_dir.iterdir() if path.is_dir()):
            if variant_dir.name == "testing":
                continue
            variant_zip = variant_dir / "latest.zip"
            if variant_zip.is_file():
                manifests.append(build_entry(variant_zip, plugin_name))
